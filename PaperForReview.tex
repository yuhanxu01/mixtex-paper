...
%%%%%%%%% ABSTRACT
\begin{abstract}
LaTeX OCR enables digitization of scientific documents but current systems rely on large curated datasets such as arXiv, making data collection expensive. For individual researchers and low-resource languages like Chinese where LaTeX source files are scarce, collecting sufficient real training data is impractical. We propose MixTex, which trains on artificially constructed mixed text--formula data requiring no manual document collection. Real natural language text from Wikipedia is randomly paired with mathematical formulas from im2latex-100k, generating 120M tokens in approximately 2 hours on consumer hardware. After training on 1M artificially constructed samples, we fine-tune on only 500 real LaTeX documents. Evaluation on 500 independently collected real samples from textbooks, lecture notes, and slides (spanning elementary school through university level, Chinese and English) shows MixTex achieves competitive or superior performance compared to Nougat trained on millions of arXiv papers, particularly on out-of-distribution educational materials. Training requires 16 hours on a single RTX 4090. This demonstrates that artificially constructed data combined with minimal fine-tuning enables high-quality LaTeX OCR without extensive real document collection. Code, model weights, and evaluation benchmarks will be publicly released.
\end{abstract}
...
LaTeX documents, despite their high clarity, the presence of complex and variable formulas demands extreme precision in text recognition. State-of-the-art systems like Nougat~\cite{blecher2023nougat} rely on large-scale arXiv datasets containing millions of LaTeX source files. However, this data-centric approach faces critical limitations: (1) collecting and annotating real LaTeX documents requires weeks of manual effort—in our pilot study, collecting just 100 real samples required 3 hours, suggesting 500 samples would require over 20 hours of manual work; (2) arXiv is predominantly university-level research papers in English, creating a distribution mismatch when applying these models to educational materials (textbooks, lecture notes, slides) or other languages; and (3) for languages like Chinese, publicly available LaTeX source files are extremely scarce, making traditional data collection approaches impractical for individual researchers.

The data scarcity problem is particularly acute for non-arXiv domains. While English research papers are abundantly available through arXiv, educational materials (elementary/middle/high school textbooks, teacher notes, presentation slides) are rarely available in LaTeX source format. A preliminary survey of major Chinese academic databases (CNKI, Wanfang) reveals that fewer than 1\% of papers provide LaTeX source files. For educational publishers, LaTeX sources are proprietary and inaccessible. This creates a critical gap: existing models trained on arXiv perform well on research papers but poorly on educational materials that constitute the majority of real-world LaTeX OCR use cases.

To address the data collection bottleneck, we propose training on \textit{artificially constructed mixed text--formula data} that can be generated automatically without manual document collection. Real natural language text from Wikipedia is randomly paired with mathematical formulas from existing collections (im2latex-100k~\cite{deng2017image}), creating documents where text and formulas lack semantic coherence. This approach offers three key advantages: (1) \textbf{Cost efficiency}: generating 1M training samples requires approximately 2 hours on consumer hardware compared to weeks of manual collection; (2) \textbf{Language extensibility}: Wikipedia provides text in 300+ languages, enabling straightforward extension to low-resource languages; and (3) \textbf{Domain generalization}: by avoiding domain-specific text-formula correlations (e.g., ``exponential decay'' $\rightarrow$ $e^{-t}$), the model learns to rely on visual features rather than contextual priors, potentially improving generalization to out-of-distribution domains.

We demonstrate that training on artificially constructed data, followed by fine-tuning on only 500 real LaTeX documents, achieves performance competitive with or superior to models trained on millions of arXiv papers. Critically, our approach shows stronger generalization to educational materials (textbooks, lecture notes, slides) where arXiv-trained models exhibit significant performance degradation.
%-------------------------------------------------------------------------
...
\item \textbf{RQ3 (Language Generalization):} Does artificially constructed multilingual training enable cross-language generalization?
\end{enumerate}
...
\textbf{RQ3: Language Generalization}

Table~\ref{tab:language} shows performance breakdown by language. Both MixTex and Nougat were trained on bilingual data (though Nougat's arXiv data is predominantly English), enabling comparison of cross-language generalization.

\begin{table}[t]
    \centering
    \caption{Performance by language (250 Chinese + 250 English samples).}
    \begin{tabular}{lcccc}
        \toprule
...
\textbf{Language Extensibility.} For low-resource languages where LaTeX source files are scarce, our approach is particularly valuable. Generating artificially constructed training data only requires: (1) Wikipedia text in the target language (available for 300+ languages), and (2) mathematical formulas (language-agnostic). This reduces the language extension problem from ``collect thousands of real documents'' to ``collect 500 documents for fine-tuning''—a 1000$\times$ reduction in manual effort.
...
Evaluation on 500 independently collected real samples from educational materials (elementary school through university level, Chinese and English) demonstrates that this approach achieves competitive or superior performance compared to Nougat, which was trained on millions of arXiv papers. Critically, our method shows [TBD: describe domain adaptation results] particularly on out-of-distribution educational materials where traditional arXiv-trained models struggle.

The practical impact is substantial: total development time (data generation + training) is under 17 hours on consumer hardware, compared to weeks or months of institutional effort for traditional data collection. For low-resource languages where LaTeX source files are scarce (e.g., Chinese, with <1\% of academic papers providing LaTeX sources), this approach enables LaTeX OCR development that would otherwise be impractical.
